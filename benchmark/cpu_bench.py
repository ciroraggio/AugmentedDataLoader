...
bench_w_debug_log=open('cpu_with_debug_profile.log','w+')
bench_no_debug_log=open('cpu_no_debug_profile.log','w+')

@profile(stream=bench_no_debug_log)
def cpu_bench_no_debug(dataset, augmentation_transforms, batch_size, subset_len, debug_path, blocks):
...
def cpu_bench_w_debug(dataset, augmentation_transforms, batch_size, subset_len, debug_path, blocks):
    data_loader = AugmentedDataLoader(dataset, augmentation_transforms, batch_size, subset_len, debug_path) # per debug
    for batch_data in data_loader.generate_batches():
        # inputs, labels = batch_data[0].to(device), batch_data[1].to(device)
        # print(f"Batch data shape: {batch_data.shape}")
...
if __name__ == '__main__':
    ...
    debug_path='../cpu/data/debug_path_test'
    blocks = 0

    start = time.time()
    cpu_bench_no_debug(dataset, augmentation_transforms, batch_size, subset_len, None, blocks)
    end = time.time()
    print(f"Tempo di esecuzione senza debug: {end-start} sec")

    start = time.time()
    cpu_bench_w_debug(dataset, augmentation_transforms, batch_size, subset_len, None, blocks)
    end = time.time()
